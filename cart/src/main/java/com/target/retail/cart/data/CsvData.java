...
public class CsvData<T extends Identifiable> {
    ...
    public CsvData(String fileName, Class<T> type) {
        ...
        this.dataByIdentifier = Collections.unmodifiableMap(data.stream()
    ...
    public Map<String, List<T>> mapUsingKey(Function<T, String> keyProvider) {
        ...
        for (T t : data) {
            String key = keyProvider.apply(t);
    ...
    public List<T> getAll() {
        return List.copyOf(dataByIdentifier.values());
    }
    ...
    private List<T> loadDataFromFile()  {
        ...
        try (MappingIterator<T> iterator = mapper.readerFor(type)
                .with(schema)
                .readValues(file)) {
